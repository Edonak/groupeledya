...
if (isset($_GET['sup'])) {
       ...
       $sqll=mysqli_query($connection,"DELETE FROM caisse_restaurant where id='$sup' ");
                                   ...
                                   <tbody>
                                     ...
                                     while ($garde = mysqli_fetch_assoc($requete2)) {  ?>
                                           ...
                                           <tr>
                                               ...
                                               <td><a href="boisson_operation.php?caisse=<?php echo $garde['id']; ?>"><?php echo $garde['designation']; ?> </a></td>
                                                <td> <?php echo $garde['categories']; ?></td>
                                               <td><a href="#"><?php echo $d = $garde['prix_fc']; ?> FC</a></td>
                                               
<td class="hidden-480"><?php echo $d = $garde['prix_usd']; ?>    $</td>
...
if (isset($_GET['caisse'])) {
       extract($_GET);
        ...
        $requete2 = mysqli_query($connection,"SELECT *FROM restaurant where id='$caisse' ORDER BY id DESC");
                                            while ($garde = mysqli_fetch_assoc($requete2)) {

$designation=$garde['designation']; 
$prix_usd=$garde['prix_usd'];
$prix_fc=$garde['prix_fc'];
$categories=$garde['categories'];
$qte=1;
...
$date_operation=date('Y-m-d');
  ...
  $insertion = $dbo->prepare("INSERT INTO  caisse_restaurant(designation,prix_usd,prix_fc,categories,percepteur,id_percepteur,qte,pt,date_operation) values(?,?,?,?,?,?,?,?,?)");

              $insertion->execute(array($designation,$prix_usd,$prix_fc,$categories,$username,$id,$qte,$prix_usd,$date_operation));
...
$code_facture=mt_rand();
     ...
     $requeteAAA = mysqli_query($connection,"UPDATE caisse_restaurant SET etat=1,code_facture='$code_facture' where etat=0 ");
  ...
  <tbody>
       <?php 
                                     $requete2 = mysqli_query($connection,"SELECT *FROM caisse_restaurant where etat=0  ORDER BY id DESC");
                                     while ($garde = mysqli_fetch_assoc($requete2)) {  ?>
   <tr>
                                                   ...
                                                   <td><?php echo $garde['prix_usd']; ?>$</td>
                                                    <td><?php echo $garde['prix_fc']; ?>FC</td>
                                                      ...
                                                      <td><a class="red" href="boisson_operation.php?sup=<?php echo $garde['id']; ?>"><i class="ace-icon fa fa-trash bigger-130"></i></a> </td>
  </tr>
     <?php }
...
<footer>
    <tr>
        <td>TOTAL</td>
        <td><strong>PRIX TOTAL USD:   <?php  $requete2 = mysqli_query($connection,"SELECT sum(prix_usd) as normale FROM caisse_restaurant where etat=0 ");
                                     while ($garde = mysqli_fetch_assoc($requete2)) {
...
echo number_format($nn); 
                                      ...
                                      ?>$</strong></td>
                                         ...
                                         <td><strong>PRIX TOTAL FC:   <?php  $requete2 = mysqli_query($connection,"SELECT sum(prix_fc) as normale FROM caisse_restaurant where etat=0 ");
                                     while ($garde = mysqli_fetch_assoc($requete2)) {
