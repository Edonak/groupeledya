...
if (isset($_POST['confirmer'])) {
    ...
    $requete2 = mysqli_query($connection, "SELECT * FROM abonnement WHERE code_client='$client'");
    while ($garde = mysqli_fetch_assoc($requete2)) {
    ...
    $prix=$garde['prix'];
    $reste=$garde['Argent_restant'];
    $Objectif=$garde['Objectif']; 
    $Antecedents=$garde['Antecedents']; 
    $code=$garde['code_client'];
    $argentRecu=$garde['Argent_recu'];
   

        $nbr = $nombre - 1;
            $requeteAAA = mysqli_query($connection, "UPDATE abonnement SET nmbr_jr='$nbr' WHERE code_client='$client'");
if($nbr<1){

    $date_enregistrement=date('Y-m-d');

$insertion = $dbo->prepare("INSERT INTO  archiveabonnement(nom_client,Date_de_naisance,sexe,telephone,adresse,taille,poids,code_client,prenom,Objectif,Antecedents,date_enregistrement,libelle,prix,nmbr_jr) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
if($reste>0){
    $insertion1 = $dbo->prepare("INSERT INTO  detteclient(codeDette,nomClient,montant_a_payer,montant_payer,montant_restant,date_enregistrement) values(?,?,?,?,?,?)");

$insertion1->execute(array($code,$nom,$prix,$argentRecu,$reste,$date_enregistrement));
}
     $requeteAAA1 = mysqli_query($connection, "UPDATE client SET etat=0 WHERE code_client='$client'");
    $requeteAAA1=mysqli_query($connection,"DELETE FROM abonnement where code_client='$client' ");
...
<script type="text/javascript">
    ...
    setInterval(function () {
        window.location.replace('presence.php');
    }, 1000);
</script>
